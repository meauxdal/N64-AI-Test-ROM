#include "audio_tests.h"
#include <libdragon.h>
#include <regsinternal.h>
#include <n64sys.h>
#include <stdint.h>
#include <math.h>

// AI Register Definitions
#define AI_BASE          0xA4500000
#define AI_DRAM_ADDR     (AI_BASE + 0x00)
#define AI_LEN           (AI_BASE + 0x04)
#define AI_CONTROL       (AI_BASE + 0x08)
#define AI_STATUS        (AI_BASE + 0x0C)
#define AI_DACRATE       (AI_BASE + 0x10)
#define AI_BITRATE       (AI_BASE + 0x14)

#define IO_READ(addr)       (*(volatile uint32_t*)(addr))
#define IO_WRITE(addr, val) (*(volatile uint32_t*)(addr) = (val))
#define PHYS_ADDR(x)        ((uint32_t)(x) & 0x1FFFFFFF)

#ifndef CLOCKRATE
#define CLOCKRATE 48681812
#endif

// 8-byte alignment is mandatory for AI DMA
static int16_t pcm_buffer[8192] __attribute__((aligned(8)));

// --- Sequence Definitions ---

static test_config_t basic_sweep[] = {
    { 22050, 0x3FFF, 16, 500 },
    { 32000, 0x3FFF, 16, 500 },
    { 44100, 0x3FFF, 16, 500 },
    { 48000, 0x3FFF, 16, 500 }
};

static test_config_t volume_test[] = {
    { 44100, 0x1000, 32, 300 },
    { 44100, 0x2000, 32, 300 },
    { 44100, 0x4000, 32, 300 },
    { 44100, 0x7FFF, 32, 300 }
};

static test_sequence_t sequences[] = {
    {
        "Standard Frequency Sweep",
        "4 tests: 22k, 32k, 44.1k, 48k @ 50% Amp",
        basic_sweep,
        4
    },
    {
        "Amplitude Scaling",
        "4 tests: Incremental volume @ 44.1kHz",
        volume_test,
        4
    }
};

// --- Core Logic ---

const test_sequence_t* get_test_sequences(int *count) {
    *count = sizeof(sequences) / sizeof(test_sequence_t);
    return sequences;
}

void calculate_dac_rates(uint32_t frequency, uint32_t *dacrate, uint32_t *bitrate) {
    // Formula: (2 * SystemClock / Frequency + 1) / 2
    *dacrate = (uint32_t)(((2.0 * CLOCKRATE / frequency) + 1) / 2);
    // Bitrate = dacrate / 66 (clamped to 16 max)
    *bitrate = (*dacrate < 66) ? 16 : (*dacrate / 66);
    if (*bitrate > 16) *bitrate = 16;
}

void trigger_audio(void* buffer, uint32_t length, uint32_t dacrate, uint32_t bitrate) {
    // 1. Set rates
    IO_WRITE(AI_DACRATE, dacrate - 1);
    IO_WRITE(AI_BITRATE, bitrate - 1);
    
    // 2. Setup DMA
    data_cache_hit_writeback_invalidate(buffer, length * 2);
    IO_WRITE(AI_DRAM_ADDR, PHYS_ADDR(buffer));
    IO_WRITE(AI_LEN, length * 2); // Length in bytes
    
    // 3. Start AI
    IO_WRITE(AI_CONTROL, 1);
}

void wait_ai_busy() {
    // Wait for the AI_STATUS busy bit (Bit 31) to clear
    while (IO_READ(AI_STATUS) & 0x80000000);
}

int wait_ms_with_abort(uint32_t ms) {
    uint32_t start = timer_ticks();
    uint32_t wait_ticks = TICKS_FROM_MS(ms);
    
    while (timer_ticks() - start < wait_ticks) {
        joypad_poll();
        joypad_buttons_t keys = joypad_get_buttons_pressed(JOYPAD_PORT_1);
        
        if (keys.b) {
            return 1; // Abort signaled
        }
    }
    return 0; // Finished normally
}

int run_single_test(int sequence_id, int test_index) {
    if (sequence_id < 0 || sequence_id >= (int)(sizeof(sequences) / sizeof(test_sequence_t)))
        return 0;
    
    test_sequence_t *seq = &sequences[sequence_id];
    if (test_index < 0 || test_index >= seq->test_count)
        return 0;
    
    test_config_t *test = &seq->tests[test_index];
    
    // Setup rates
    uint32_t dacrate, bitrate;
    calculate_dac_rates(test->frequency, &dacrate, &bitrate);
    
    // Fill buffer with constant tone (Square pulse)
    for (int j = 0; j < test->sample_count; j++) {
        pcm_buffer[j] = test->amplitude;
    }
    
    // Play
    trigger_audio(pcm_buffer, test->sample_count, dacrate, bitrate);
    wait_ai_busy();
    
    // Return the abort status from the post-play wait
    return wait_ms_with_abort(test->wait_ms);
}