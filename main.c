...
static void draw_menu(surface_t *disp) {
    ...
    // IRQ test as option 4
    {
        ...
        graphics_set_color(text_color, 0);
        char line[48];
        snprintf(line, sizeof(line), "%s %d. AI IRQ Timing Test",
                 (i == menu_selection) ? ">" : " ", i + 1);
        graphics_draw_text(disp, 30, 60 + i * 35, line);
        graphics_set_color(graphics_make_color(130, 130, 130, 255), 0);
        graphics_draw_text(disp, 45, 75 + i * 35, "PASS on hardware, FAIL on ares");
    }

    graphics_set_color(graphics_make_color(200, 200, 200, 255), 0);
    graphics_draw_text(disp, 25, 215, "DPAD: Select  A: Run");
}
...
int main(void) {
    ...
    int total_options = seq_count + 1;  // +1 for IRQ test

    while (1) {
        if (running_irq_test) {
            // Run the IRQ test, display result, wait for A (rerun) or B (menu)
            irq_test_result_t result = run_irq_test();

            while (1) {
                surface_t *disp = display_get();
                draw_irq_result(disp, &result);
                display_show(disp);

                joypad_poll();
                joypad_buttons_t keys = joypad_get_buttons_pressed(JOYPAD_PORT_1);
                if (keys.a) {
                    // Rerun — stay in running_irq_test, loop will re-execute
                    break;
                }
                if (keys.b) {
                    running_irq_test = 0;
            ...
            if (keys.d_down && menu_selection < total_options - 1)  menu_selection++;
            if (keys.a) {
                if (menu_selection == seq_count) {
                    // IRQ test selected
                    running_irq_test = 1;
                } else {
                    running_test = 1;
            ...
            const test_sequence_t *sequences = get_test_sequences(NULL);
